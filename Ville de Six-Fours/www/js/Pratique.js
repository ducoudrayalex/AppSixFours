            ...
            success: function (xml) {
                ...
                $('.liencontenuPratique').click(function () {
                    ...
                    $('#PageContenuPratique').on("pageshow", function () {
                        positionService = new google.maps.LatLng(stockElementXML[id].lat, stockElementXML[id].long);
                        mapOptions = {//options de la map (zoom,centrage)
                            zoom: 14,
                            center: positionService
                        };
                        map = new google.maps.Map($('#map_canvas')[0], mapOptions);//creation de la map dans l'element html souhaité

                        markerService = new google.maps.Marker({//ajout d'un marker pour indiquer ou se trouve le service
                            position: positionService, //latitude et longitude que l'on recupere dans le fichier xml XMLPratique
                            map: map,
                            title: stockElementXML[id].nom + '\n' + stockElementXML[id].adresse
                        });
                        //MaPosition(map);
                        infowindowService = new google.maps.InfoWindow({//fenetre d'information au dessus du marker pour indiquer le nom du service entre autre
